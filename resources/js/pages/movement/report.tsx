...
interface MovementSummary {
    ...
    rejected?: number; // For backward compatibility
...
export default function MovementReport({
    ...
    const handleDownloadPdf = () => {
        ...
        window.open(`${route('movements.report.download')}?${params.toString()}`, '_blank');
    };
    ...
    // Map old summary keys to new ones for backward compatibility
    const activeTotalCount = summary?.active || 0;
    const completedTotalCount = summary?.completed || 0;
                        ...
                        <CardContent className="pt-6">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold mt-1">{activeTotalCount}</p>
                                </div>
                                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                                    {calculatePercentage(activeTotalCount, summary?.total || 1)}%
                                </Badge>
                            </div>
                            <div className="mt-4 w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-blue-500 rounded-full"
                                    style={{ width: `${calculatePercentage(activeTotalCount, summary?.total || 1)}%` }}
                                ></div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="pt-6">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold mt-1">{completedTotalCount}</p>
                                </div>
                                <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                                    {calculatePercentage(completedTotalCount, summary?.total || 1)}%
                                </Badge>
                            </div>
                            <div className="mt-4 w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-green-500 rounded-full"
                                    style={{ width: `${calculatePercentage(completedTotalCount, summary?.total || 1)}%` }}
                                ></div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="text-3xl font-bold mt-1 flex items-center">
                                        ...
                                        <p>
                                            {movements?.data?.filter(m => m.status === 'completed' && m.actual_return_datetime)?.length > 0 ? (
                                                calculateAverageDuration(movements.data)
                                            ) : (
                                                'N/A'
                                            )}
                                        </p>
                            ...
                            <div className="mt-4 text-xs text-gray-500">
                                Based on completed movements with actual return time data
                            </div>
                        </CardContent>
                    </Card>
                </div>
...
function calculateAverageDuration(movements: Movement[]) {
    const completedMovements = movements.filter(m =>
        m.status === 'completed' && m.actual_return_datetime
    );

    if (completedMovements.length === 0) return 'N/A';
    ...
    completedMovements.forEach(movement => {
        const fromTime = new Date(movement.from_datetime);
        const returnTime = new Date(movement.actual_return_datetime!);
        const diffMinutes = differenceInMinutes(returnTime, fromTime);
        totalMinutes += diffMinutes;
    });

    const avgMinutes = Math.round(totalMinutes / completedMovements.length);
