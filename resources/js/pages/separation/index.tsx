...
import { Check, ChevronLeft, ChevronRight, Eye, Pencil, Plus, Search, Trash2, UserX } from 'lucide-react';
...
export default function SeparationIndex({ separations, employees, filters, canEditSeparations = false, canDeleteSeparations = false }: Props) {
    ...
    const confirmDelete = (s: Separation) => {
        const name = employeeDisplayName(s.employee);
        const message = s.status === 'completed'
            ? `Delete this completed separation for ${name}?\n\nThis will restore the employee to active, clear dropout details, reverse final-payment settlements (PF / gratuity / loans when applied), and remove the separation record.`
            : `Delete this separation record for ${name}?`;
        if (!confirm(message)) {
            return;
        }
        router.delete(route('separations.destroy', s.id));
    };
                                    ...
                                    <TableRow key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                                        ...
                                        <TableCell className="pr-6 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                {canDeleteSeparations && (
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg bg-rose-50 text-rose-600" title="Delete separation" onClick={() => confirmDelete(s)}><Trash2 className="h-4 w-4" /></Button>
                                                )}
                                            </div>
                                        </TableCell>
                                    </TableRow>
