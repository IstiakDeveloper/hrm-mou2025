  ...
  return (
    <Layout>
      ...
      <PageSurface>
        ...
        {/* Pagination - Fixed version similar to Employee Index */}
        {hasPagination && (
          <div className="flex items-center justify-between border-t px-4 py-3 mt-6">
            <div className="flex flex-1 items-center justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{holidays.meta.from}</span> to{' '}
                  <span className="font-medium">{holidays.meta.to}</span>{' '}
                  of <span className="font-medium">{holidays.meta.total}</span> holidays
                </p>
              </div>
              <div>
                <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                  {holidays.meta.current_page > 1 && (
                    <Link
                      href={route('holidays.index', {
                        page: holidays.meta.current_page - 1,
                        search,
                        year: selectedYear
                      })}
                  ...
                  {holidays.meta.links.filter(link =>
                    !link.label.includes('&laquo;') && !link.label.includes('&raquo;')
                  ).map((link, i) => {
                    // Skip ellipsis as we can handle it properly
                    if (link.label === '...') {
                      return (
                        <span
                          key={i}
