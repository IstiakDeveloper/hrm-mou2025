...
import { Edit, Plus, Search, Trash2, X, ChevronLeft, ChevronRight, Layers } from 'lucide-react';
...
export default function ProgramsIndex({ programs, filters }: Props) {
    ...
    const handleSearch = () => {
        router.get(route('programs.index'), { search, per_page: perPage }, { preserveState: true });
    };

    const handlePerPageChange = (value: string) => {
        setPerPage(value);
        router.get(route('programs.index'), { search, per_page: value }, { preserveState: true });
    };

    const typeLabel = (t: string) => (t === 'project' ? 'Project' : 'Core');
            ...
            <PageSurface>
                <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-5">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">Core / project programs linked on employee records</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
                        <div className="relative w-full sm:w-64">
                            ...
                            <Input
                                ...
                                onKeyDown={(e) => e.key === 'Enter' && router.get(route('programs.index'), { search }, { preserveState: true })}
                                className="pl-9 h-9 text-sm bg-white border-slate-200 focus-visible:ring-emerald-500 rounded-lg transition-all"
                            />
                            {search && (
                                <button
                                    onClick={() => { setSearch(''); router.get(route('programs.index')); }}
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            <Button onClick={() => router.get(route('programs.index'), { search }, { preserveState: true })} size="sm" className="h-9 w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700">Search</Button>
                            <Link href={route('programs.create')} className="w-full sm:w-auto">
                                    ...
                                    {programs.data.length ? (
                                        programs.data.map((row) => (
                                            <TableRow key={row.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 group">
                                                ...
                                                <TableCell className="text-[13px] text-slate-600 font-medium">{typeLabel(row.type)}</TableCell>
                                                <TableCell>
                                                    {row.is_active ? (
                                                ...
                                                <TableCell className="text-right pr-6">
                                                    <div className="flex items-center justify-end gap-2 transition-opacity duration-200">
                                                        <Button 
                                                            ...
                                                            title="Edit Program"
                                                            onClick={() => router.get(route('programs.edit', row.id))}
                                                        >
