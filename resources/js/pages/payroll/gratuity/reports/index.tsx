...
export default function GratuityReportsIndex({ reports }: { reports: ReportItem[] }) {
    return (
        <StaffFundLayout
            title="Gratuity Reports"
            description="Generate, print, PDF or Excel (CSV) — same style as PF and payroll reports."
            activeTab="gratuity-reports"
        >
            <Head title="Gratuity Reports" />

            <div className="rounded-lg border border-emerald-100 bg-white shadow-2xs">
                <div className="border-b border-emerald-50 px-4 py-3">
                    <h2 className="text-sm font-bold text-zinc-800">All gratuity reports</h2>
                ...
                <ul className="divide-y divide-slate-200">
                    {reports.map((r) => (
                        <li key={r.slug}>
                            <Link
                                href={gratuityReportPath(r.slug)}
