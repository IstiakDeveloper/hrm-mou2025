...
export default function EmployeeCreate({
    ...
    const getTabStepState = (tabId: EmployeeCreateTabId) => {
        ...
        const hasServerErrors = Object.keys(errors).some(
            (k) => errors[k] && errorFieldKeyToEmployeeCreateTab(k) === tabId
        );
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-100 pb-6 mb-8">
                    <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-zinc-900">Add New Employee</h1>
                                ...
                                <div className="lg:hidden rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm">
                                    <div className="flex items-center justify-between gap-4">
                                        <div>
                                            ...
                                            <h3 className="text-sm font-bold text-zinc-800 mt-0.5">{tabLabels[activeTab as EmployeeCreateTabId]?.title}</h3>
                                        </div>
                                        <div className="text-right text-xs font-bold text-emerald-600">
                                            {EMPLOYEE_CREATE_TAB_ORDER.indexOf(activeTab as EmployeeCreateTabId) + 1} / {EMPLOYEE_CREATE_TAB_ORDER.length}
                                        </div>
                                    </div>
                                    <div className="w-full bg-zinc-100 h-1.5 rounded-full mt-3 overflow-hidden">
                                        <div className="bg-emerald-600 h-full rounded-full transition-all duration-300" style={{ width: `${((EMPLOYEE_CREATE_TAB_ORDER.indexOf(activeTab as EmployeeCreateTabId) + 1) / EMPLOYEE_CREATE_TAB_ORDER.length) * 100}%` }} />
                                    </div>
                                ...
                                <Tabs value={activeTab} onValueChange={requestTabChange} className="w-full">
                                                        ...
                                                        {data.guarantor_cheques.map((ch, idx) => (
                                                            <div key={idx} className="rounded-2xl border border-zinc-200 p-4 bg-white space-y-3 relative hover:border-zinc-300 transition-colors shadow-sm">
                                                                <div className="flex items-center justify-between border-b border-zinc-50 pb-1.5">
                                                                    ...
                                                                    <Button type="button" variant="ghost" size="sm" className="h-6.5 text-xs text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg px-2" onClick={() => setData('guarantor_cheques', data.guarantor_cheques.filter((_, i) => i !== idx))}><Trash2 className="h-3 w-3 mr-1" /> Remove</Button>
                                                                </div>
                                                    ...
                                                    {data.assets.map((as, idx) => (
                                                        <div key={idx} className="rounded-2xl border border-zinc-200 bg-white p-5 space-y-4 hover:border-zinc-300 transition-all duration-150 shadow-sm">
                                                            ...
                                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                                                                ...
                                                                <FormField label="Assignment Details" className="sm:col-span-2 lg:col-span-5"><Input value={as.asset_details} onChange={(e) => { const next = [...data.assets]; next[idx] = { ...next[idx], asset_details: e.target.value }; setData('assets', next); }} placeholder="Describe condition or allocation notes..." /></FormField>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                    ...
                                    <TabsContent value="documents" className="mt-0 focus-visible:outline-none">
                                                                ...
                                                                <FormField label="Document File" error={errors[`documents.${idx}.file`]}>
                                                                    <div className="flex items-center gap-2 mt-0.5">
                                                                        <label className="inline-flex cursor-pointer items-center justify-center gap-1.5 h-10 px-3.5 border border-zinc-200 rounded-lg text-xs font-bold hover:bg-zinc-50 transition-colors w-full text-zinc-600 shadow-sm">
                                                                            ...
                                                                            <span className="truncate max-w-[120px]">{doc.file ? doc.file.name : 'Choose File'}</span>
                                        ...
                                        <div className="border-t border-zinc-100 bg-zinc-50/50 p-6 flex justify-between items-center rounded-b-2xl">
                                            ...
                                            <Button type="submit" disabled={processing} className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 rounded-lg shadow-sm h-10">{processing ? 'Creating Profile…' : 'Create Profile'}</Button>
                                        </div>
                                    </TabsContent>
