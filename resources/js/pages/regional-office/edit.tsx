...
export default function RegionalOfficeEdit({ regionalOffice, zones, employees }: Props) {
  const { data, setData, put, processing, errors } = useForm({
    zone_id: String(regionalOffice.zone_id),
    name: regionalOffice.name || '',
    code: regionalOffice.code || '',
    description: regionalOffice.description || '',
    is_active: Boolean(regionalOffice.is_active),
    regional_manager_employee_id: regionalOffice.regional_manager_employee_id ? String(regionalOffice.regional_manager_employee_id) : null as string | null,
  });

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('regional-offices.update', regionalOffice.id));
  };

  return (
    <Layout>
      <Head title={`Edit Regional Office: ${regionalOffice.name}`} />
      ...
      <div className="container mx-auto py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Edit Regional Office</h1>
          <p className="mt-1 text-gray-500">Update regional office information</p>
        </div>
        ...
        <form onSubmit={submit}>
          <Card className="max-w-3xl mx-auto">
            <CardHeader className="border-b bg-gray-50">
              ...
              <CardDescription>Update regional office details</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-6">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setData('zone_id', value === 'null' ? '' : value)}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={data.regional_manager_employee_id || 'none'}
                  onValueChange={(value) => setData('regional_manager_employee_id', value === 'none' ? null : value)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select regional manager (optional)" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <CardFooter className="border-t bg-gray-50 px-6 py-4 flex justify-end">
              <div className="flex space-x-2">
                ...
                <Button type="submit" disabled={processing} className="bg-green-600 hover:bg-green-700">
                  {processing ? 'Updating...' : 'Update Regional Office'}
                </Button>
              </div>
            </CardFooter>
          </Card>
