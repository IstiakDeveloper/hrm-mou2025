...
class ImportPfLoanFromXlsxCommand extends Command
{
    protected $signature = 'loans:import-pf-loan
                            {--path= : Absolute or project-relative XLSX path (default: data/excel/pfloan.xlsx)}
                            {--closing-date= : Migration closing date (default: today)}
                            {--dry-run : Parse and log only; no database writes}';

    protected $description = 'Import running PF loans from PIN-keyed spreadsheet (pfloan.xlsx) into loan migration.';
