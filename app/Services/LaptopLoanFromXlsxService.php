...
/**
 * Import running laptop loans from HR spreadsheet (laptoploan.xlsx) via loan migration batch.
 */
class LaptopLoanFromXlsxService
{
    private const DEFAULT_XLSX = 'data/excel/laptoploan.xlsx';

    /** @var array<int, string> */
    private const POLICY_CODE_BY_DISBURSE = [
        40000 => '015',
        35000 => '019',
        32000 => '017',
        31000 => '018',
    ...
    private const MONTHLY_COL_START = 8;

    private const MONTHLY_COL_END = 19;
    ...
    public function run(?string $xlsxAbsolutePath = null, bool $dryRun = false, ?string $closingDate = null): array
    {
        ...
        $policyByCode = LoanPolicy::query()
            ->whereIn('code', array_values(self::POLICY_CODE_BY_DISBURSE))
            ->get()
            ->keyBy('code');
        ...
        foreach ($rows as $row) {
            ...
            if ($nameKey === 'total' || str_contains($nameKey, 'grand total')) {
                $skippedSummaryRow++;
            ...
            $policyCode = $this->resolvePolicyCode($disburse);
        ...
        $logPath = storage_path('logs/laptop-loan-xlsx-'.date('Y-m-d_His').'.log');
    ...
    private function resolvePolicyCode(int $disburse): ?string
    {
        return self::POLICY_CODE_BY_DISBURSE[$disburse] ?? null;
    }

    private function inferInstallmentAmount(int $disburse): int
    {
        return match ($disburse) {
            40000 => 1500,
            35000 => 1500,
            32000 => 1450,
            31000 => 1450,
    ...
    private function parseSpreadsheetAmount(mixed $value): float
    {
        ...
        $firstToken = preg_split('/\s+/', $raw)[0] ?? '';
        $clean = preg_replace('/[^\d.]/', '', str_replace(',', '', $firstToken));
    ...
    private function buildVerification(float $xlsxDisburseTotal, float $xlsxOutstandingTotal, bool $dryRun): array
    {
        ...
        $laptopLoans = EmployeeLoan::query()
            ->where('is_legacy_import', true)
            ->where('status', 'active')
            ->whereIn('loan_policy_id', LoanPolicy::query()->where('loan_type', 'laptop_loan')->select('id'));

        $dbDisburse = SalaryStructureCalculator::roundTaka((float) (clone $laptopLoans)->sum('principal_amount'));
        $dbOutstanding = SalaryStructureCalculator::roundTaka((float) (clone $laptopLoans)->sum('outstanding_balance'));
    ...
    private function parseXlsx(string $absPath): array
    {
        ...
        if (! isset($col['pin'], $col['disburse_amount'], $col['current_balance'])) {
            throw new InvalidArgumentException(
                'Spreadsheet must include Pin, Disbursement Taka, and Current Balance columns (laptoploan.xlsx layout).'
            );
        }
        ...
        foreach (array_slice($sheetRows, 1) as $offset => $row) {
            ...
            $installmentAmount = '';
            for ($i = self::MONTHLY_COL_START; $i <= self::MONTHLY_COL_END; $i++) {
                $cell = $this->parseSpreadsheetAmount($row[$i] ?? '');
                if ($cell > 0) {
            ...
            $out[] = [
                ...
                'disburse_amount' => (string) $this->parseSpreadsheetAmount($row[$col['disburse_amount']] ?? ''),
                'installment_amount' => $installmentAmount,
                'current_balance' => (string) $this->parseSpreadsheetAmount($row[$col['current_balance']] ?? ''),
            ];
        }
