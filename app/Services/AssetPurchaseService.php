...
class AssetPurchaseService
{
    ...
    /**
     * Delete purchase and its associated assets and items.
     */
    public function deletePurchase(AssetPurchase $purchase): void
    {
        DB::transaction(function () use ($purchase) {
            $assets = FixedAsset::query()->where('asset_purchase_id', $purchase->id)->get();
            ...
            foreach ($purchase->items as $item) {
                if ($item->photo_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($item->photo_path)) {
                    \Illuminate\Support\Facades\Storage::disk('public')->delete($item->photo_path);
                }
                $item->delete();
            }
    ...
    /**
     * Update an existing purchase header and its items / fixed assets.
     */
    public function updatePurchase(AssetPurchase $purchase, array $header, array $items, array $photos, ?int $userId): AssetPurchase
    {
        return DB::transaction(function () use ($purchase, $header, $items, $photos, $userId) {
            $branch = Branch::query()->findOrFail($header['branch_id']);
            $purchaseTotal = 0;
            ...
            foreach ($items as $index => $itemData) {
                ...
                $subCategory = !empty($itemData['asset_sub_category_id'])
                    ? AssetSubCategory::query()->find($itemData['asset_sub_category_id'])
                    : null;
                ...
                if ($itemId && in_array($itemId, $existingItemIds, true)) {
                    $purchaseItem = AssetPurchaseItem::query()->find($itemId);
                    $itemUpdate = [
                        'asset_category_id' => $category->id,
                        'asset_sub_category_id' => $itemData['asset_sub_category_id'] ?? null,
                        'quantity' => $quantity,
                    ...
                    if ($photoPath) {
                        $itemUpdate['photo_path'] = $photoPath;
                    }
                    $purchaseItem->update($itemUpdate);
                    $processedItemIds[] = $itemId;

                    $assets = FixedAsset::query()->where('asset_purchase_item_id', $purchaseItem->id)->get();
                    $manualCodes = $itemData['manual_asset_codes'] ?? [];

                    foreach ($assets as $codeIndex => $asset) {
                        $manualCode = !empty($manualCodes[$codeIndex]) ? Str::upper(trim((string) $manualCodes[$codeIndex])) : $asset->manual_asset_code;
                        $assetUpdate = [
                            'name' => count($assets) > 1 ? "{$assetName} (#".($codeIndex + 1).')' : $assetName,
                            'manual_asset_code' => $manualCode,
                            'asset_category_id' => $category->id,
                            'asset_sub_category_id' => $subCategory?->id,
                            'branch_id' => $branch->id,
                        ...
                        if ($photoPath) {
                            $assetUpdate['photo_path'] = $photoPath;
                        }
            ...
            $toDeleteIds = array_diff($existingItemIds, $processedItemIds);
            if (! empty($toDeleteIds)) {
                FixedAsset::query()->whereIn('asset_purchase_item_id', $toDeleteIds)->delete();
                AssetPurchaseItem::query()->whereIn('id', $toDeleteIds)->delete();
            }
